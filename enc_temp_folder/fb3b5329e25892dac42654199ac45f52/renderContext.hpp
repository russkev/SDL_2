...
namespace graphics {
	...
	public:
		...
		void clip_polygon_component(const std::vector<vertex>& vertex_list, int component, float factor) {
			vertex previous_vertex = vertex_list.back();
			float previous_component = previous_vertex.get(component) * factor;
			bool previous_clip = (previous_component <= previous_vertex.m_pos.w);
