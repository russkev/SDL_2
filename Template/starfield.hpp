...
namespace graphics {
	struct starfield {
		...
		starfield (int numStars, float speed, float spread, float fog_dist):
		m_speed(speed), m_spread(spread), m_fog_dist(fog_dist){
	...
	private:
		float m_spread, m_speed, m_fog_dist;
	...
	void starfield::render_stars(_View& s_view, double s_delta_time) {
		...
		int count = 0;
		float alpha = 255, alpha_multiplier = alpha/m_fog_dist;
		...
		for (auto& star : m_star) {
			...
			float it_x = (star.x/star.z) * half_width + half_width;
			float it_y = (star.y/star.z) * half_height + half_width;
			...
			if (it_x < 0 || it_x > s_view.size().x-1 ||   //1024
				...
				blend_element(s_view, tvec2<int>(round(it_x), round(it_y)), tvec4<std::uint8_t>(0, 0, 255, alpha));
			}
		}
	}
}