...
int main(int, char**) try {
	...
	// Initialise fps
	fps frame_check(s_window, 1.0 / 8.0, 0.1);

	auto s_time0 = s_freq_multiplier * SDL_GetPerformanceCounter();
	auto s_time1 = s_time0;
	while (!SDL_QuitRequested()) {
		const auto s_time1 = s_freq_multiplier * SDL_GetPerformanceCounter();
		const auto s_delta_time = s_time1 - s_time0;
		s_time0 = s_time1;
		frame_check.think(s_delta_time);
		...
		draw_animation_frame(s_surface[0], s_time1, s_delta_time, projection);
